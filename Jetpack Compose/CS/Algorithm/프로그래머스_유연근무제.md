...
class Solution {
    fun solution(schedules: IntArray, timelogs: Array<IntArray>, startday: Int): Int {
        ...
        for (i in schedules.indices) {
            if (isWinner(schedules[i], timelogs[i], startday)) answer++
        }
        return answer
    }

    private fun toMinute(time: Int): Int {
        val hour = time / 100
        val minute = time % 100
        return hour * 60 + minute
    }
    
    private fun isWinner(schedule: Int, logs: IntArray, startDay: Int): Boolean {
        val deadlineMinutes = toMinute(schedule) + 10
        
        var successCount = 0
        for (index in logs.indices) {
            // 0: 월, 1: 화, ..., 6: 일
            val dayOfWeek = (startDay - 1 + index) % 7
            val isWeekDay = dayOfWeek in 0..4
            
            if (isWeekDay) {
                val logMinutes = toMinute(logs[index])
                if (logMinutes <= deadlineMinutes) successCount++
            }
        }
