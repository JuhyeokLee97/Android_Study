...
class Solution {
    fun solution(X: String, Y: String): String {
        val numXMap = mutableMapOf<Char, Int>()
        val numYMap = mutableMapOf<Char, Int>()

        for (char in X) {
            numXMap[char] = numXMap.getOrDefault(char, 0) + 1
        }
        for (char in Y) {
            numYMap[char] = numYMap.getOrDefault(char, 0) + 1
        }

        val keySet = numXMap.keys.filter { it in numYMap.keys }
        if (keySet.isEmpty()) return "-1"

        val keyList = keySet.toList().sortedByDescending { it }
        val sb = StringBuilder()

        for (key in keyList) {
            val numX = numXMap.getOrDefault(key, 0)
            val numY = numYMap.getOrDefault(key, 0)
            val num = min(numX, numY)
            sb.append(key.toString().repeat(num))
        }
